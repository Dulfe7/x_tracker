...
class RateLimitError(RuntimeError):
    """Raised when the X API responds with a rate-limit error."""

    def __init__(self, wait_seconds: Optional[int], message: str):
    ...
    try:
        ...
        if response.status_code == 429:
            ...
            raise RateLimitError(
                wait_seconds,
                f"X API rate limit exceeded ({response.status_code}): {response.text}",
            ) from exc
        raise RuntimeError(
    ...
    def check_once(self) -> None:
        for username in self.config.accounts:
            try:
                ...
                wait_note = (
                    f" Подождите около {exc.wait_seconds} секунд перед повтором."
                    if exc.wait_seconds is not None
                    else " Подождите немного и запустите снова."
                )
                print(f"Достигнут лимит запросов X для @{username}.{wait_note}")
                continue
            except RuntimeError as exc:
                print(f"Пропуск @{username} из-за ошибки: {exc}")
                continue
        self._persist_state()
